...
.meta-body.open { max-height: 4000px; }
.meta-content {
  padding: 0 0 1.1rem;
  font-size: .85rem;
  color: var(--text-secondary);
...
<div class="container py-4">
    ...
    <div class="meta-accordion">
      ...
      @if($product->weight || $product->brand)
        <div class="meta-item">
          <button class="meta-trigger" onclick="toggleMeta(this)">
            Specifications
            <span class="meta-icon fa-solid fa-plus"></span>
      ...
      @endif
        ...
        <div class="meta-item">
            <button class="meta-trigger" onclick="toggleMeta(this)">
            Reviews & Ratings ({{ $product->review_count }})
            <span class="meta-icon fa-solid fa-plus"></span>
            </button>
            <div class="meta-body">
            <div class="meta-content">
...
function pickStar(val) {
    ...
    document.querySelectorAll('.star-pick-btn').forEach((btn, i) => {
        const isActive = (i + 1) <= val;
        btn.style.color     = isActive ? 'var(--warning)' : 'var(--border)';
        btn.style.transform = isActive ? 'scale(1.2)' : 'scale(1)';
    });
}

// Hover effect
document.querySelectorAll('.star-pick-btn').forEach((btn, i, all) => {
    btn.addEventListener('mouseenter', () => {
        const val = parseInt(btn.dataset.val);
        all.forEach((b, j) => {
            b.style.color = (j + 1) <= val ? 'var(--warning)' : 'var(--border)';
        });
    });
    btn.addEventListener('mouseleave', () => {
        const current = parseInt(document.getElementById('ratingInput').value) || 0;
        all.forEach((b, j) => {
            b.style.color = (j + 1) <= current ? 'var(--warning)' : 'var(--border)';
        });
    });
});

// session message থাকলে Reviews accordion auto-open
@if(session('review_success') || session('review_error') || $errors->any())
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.meta-trigger').forEach(trigger => {
      if (trigger.textContent.trim().startsWith('Review')) {
        toggleMeta(trigger);
        trigger.closest('.meta-item')
